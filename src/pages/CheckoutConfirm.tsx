...
export default function CheckoutConfirm() {
  ...
  const hasClearedCart = useRef(false);  // Usamos useRef para evitar el bucle
  ...
  useEffect(() => {
   ...
   // Limpiar el carrito solo cuando el pago sea exitoso y solo una vez
  useEffect(() => {
    if (paymentResult?.status === 'AUTHORIZED' && !hasClearedCart.current) {
      clearCart();  // Limpiar el carrito después de una compra exitosa
      hasClearedCart.current = true;  // Marca que ya se limpió el carrito
    }
  }, [paymentResult, clearCart]);  // Solo cuando paymentResult cambia
  ...
  if (paymentResult?.status === "AUTHORIZED") {
    const { customerName, customerEmail, address, cardLast4Digits } =
      paymentResult.customerInfo;
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-black flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-2">
            ...
            {paymentResult.customerInfo.customerName}
          </p>
          <p className="text-gray-500 dark:text-gray-400 mb-2">
